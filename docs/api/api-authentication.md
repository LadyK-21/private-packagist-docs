...
secret, that can be created in your organization settings under _API access_. All API credentials are
...
* [Required] **Key**: "API key" part of your organization's _API access_ credentials
...
* [Required] **Version**: Signature scheme version. The current signature schema version is `2`.
...
$stringToSign =
    $request->getMethod() . "\n" // all caps
    . $request->getHost() . "\n" // api host
    . $request->getPath() . "\n" // path only, query string is signed via $params['query']
...
The `query` field must always be set, even when the URL has no query string (in which case it is the empty string).
